...
class StreamProcessor:
    def __init__(self, voice_converter):
        ...
        # Hysteresis was TRIED here and REVERTED — do not reintroduce it without
        # measuring pause noise. Opening at 1.8x with a 0.5x close and a 100ms release
        # hold made pause noise 3-5x WORSE (in_100904 p95 0.000 -> 0.469), because the
        # noise in a pause is generated by RVC itself, so a release hold passes 100ms
        # of hallucinated noise after every single word. A release hold only helps when
        # the unwanted signal is external; here it IS the output.
        # These are left at 1.0/1.0/0, which is a plain per-frame threshold.
        self.gate_open_mult = 1.0
        self.gate_close_mult = 1.0
    ...
    def _gate_decision(self, env, thr):
        """
        Open/closed per frame, with hysteresis and a release hold, carried across
        blocks so seams are not special. Opening needs a clear crossing so pause
        noise cannot trigger it; closing needs a much lower level plus the hold, so
        sustained speech and the dips inside it stay open.
        """
        open_t = self.gate_open_mult * thr
        close_t = self.gate_close_mult * thr
        gate = np.zeros(len(env), dtype=np.float32)
        st, hold = self._gate_open, self._gate_hold
        for i, e in enumerate(env):
            if st:
                if e < close_t:
                    hold += 1
                    if hold > self.gate_hold_frames:
                        st, hold = False, 0
                else:
                    hold = 0
            elif e > open_t:
                st, hold = True, 0
            gate[i] = 1.0 if st else 0.0
        self._gate_open, self._gate_hold = st, hold
        return gate

    def _gate_to_input(self, output, ref, thr=None):
        """
    ...
    def process(self, audio_bytes):
        ...
        # Threshold derived from this speaker's own levels, computed ONCE here and
        # reused by the gate below — vad_threshold was another absolute constant, the
        # same mistake that made the gate delete speech on quiet input.
        ...
        try:
            ...
            if self.gate_enabled:
                output = self._gate_to_input(output, emit_in, thr=_thr)
